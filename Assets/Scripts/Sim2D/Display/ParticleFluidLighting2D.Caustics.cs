...
namespace Seb.Fluid2D.Rendering
{
	public partial class ParticleFluidLighting2D
	{
		...
		public RenderTexture GetCausticsLightDirectionTarget()
		{
			return UsesDirectionalLightFieldResultTexture() ? lightDirectionTexture : lightDirectionResultFallbackTexture;
		}

		public bool UsesDirectionalLightFieldResultTexture()
		{
			return ShouldRenderDirectionalLightField();
		}
		...
		internal static float LightSampleWeight(DirectionalLightSettings light)
		{
		...
		internal static bool SupportsCausticRaymarch(DirectionalLightSettings light)
		{
			return light != null
			       && light.enabled
			       && light.intensity > 0f;
		...
		internal void GetCausticRayRange(FrameContext context, int width, int height, Vector3 lightDirection, out float startOffset, out int rayCount)
		{
			ParticleDisplay2D display = context.display;
		...
		internal static int GetCausticPointRayCount(DirectionalLightSettings light, Vector2 worldSize, int width, int height)
		{
			if (light == null)
			{
				return 0;
