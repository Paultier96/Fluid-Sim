Shader "Hidden/Particle2DMetaballMaterial" {
	...
	SubShader {
		...
		float3 ApplyAnalyticBoundaryNormal(float3 particleNormal, float2 worldPos)
		{
			...
			float2 analyticXY = OuterAnalyticBoundaryNormal(worldPos) * sign(metaballGhostBoundaryNormalStrength) *
				analyticMagnitude;
		...
		{
			float2 normal0XY = ApplyNormalStrength(GetPhaseNormalXY(normalPacked, density0, density1, false),
	                                            GetPhaseNormalStrength(false));
			float2 normal1XY = ApplyNormalStrength(GetPhaseNormalXY(normalPacked, density0, density1, true),
                                    GetPhaseNormalStrength(true));
		...
		float SampleAntiAliasedPhaseT(float2 uv, float density0, float density1, float blendWidth)
		{
			...
			float phaseRatioRight = ParticleFluidPhaseRatio(SamplePhase0DensityAtUv(uv + float2(texel.x, 0.0)),
				SamplePhase1DensityAtUv(uv + float2(texel.x, 0.0)));
			float phaseRatioLeft = ParticleFluidPhaseRatio(SamplePhase0DensityAtUv(uv - float2(texel.x, 0.0)),
	                                       SamplePhase1DensityAtUv(uv - float2(texel.x, 0.0)));
			float phaseRatioUp = ParticleFluidPhaseRatio(SamplePhase0DensityAtUv(uv + float2(0.0, texel.y)),
				SamplePhase1DensityAtUv(
					uv + float2(0.0, texel.y)));
			float phaseRatioDown = ParticleFluidPhaseRatio(SamplePhase0DensityAtUv(uv - float2(0.0, texel.y)),
	                         SamplePhase1DensityAtUv(uv - float2(0.0, texel.y)));
		...
		bool ResolveMetaballMaterial(v2f i, out float alpha, out float phaseT, out float data0, out float data1,
		           out float3 normal, out float3 albedo)
		{
			...
			float particleAlpha = smoothstep(max(densityThreshold - edgeSoftness, 0), densityThreshold + edgeSoftness,
	                                                       density);
			...
			normal = ApplyAnalyticBoundaryNormal(GetCombinedPhaseNormal(normalPacked, density0, density1, phaseT),
	                                                        worldPos);
			...
			float refractedPhaseT = screenSpaceRefractionCanCrossPhases != 0 && refractedDensity >= densityThreshold
		      ? step(refractedPhaseBoundary, refractedPhaseRatio)
		      : phaseT;
			albedo = ParticleFluidSampleGradientColour(refractedCombined, refractedDensity0, refractedDensity1, data0,
	                              data1, refractedPhaseT);
			return true;
		}
